...
// A] Importing Section  :
  ...
  // importing the [env] file at top of the server file  :
  ...
  // c) Importing {verifyJWT} the middleware file to be used here for protecting routes [   we will use it for access {employees route} - but not for {root} {auth} , {register} routes - ] :
    ...
    // f) Importing and  define  {mongoose}  :
    const  mongoose  = require('mongoose') ;

    
    // g) Importing the  defined connection method {connectDB} from it's file [config/dbConn]  :
...
//  B] [Express Server public configuration definitions processes steps] section :
  ...
  // 2] Connecting [MongoDB] database before any other using of middleware : 
    connectDB() ;   
...
// D] Launching the [Listen] process on the defined variable of server {app} [to be able to listen its request] :
  //  Difine the Listen event within mongoose connection with using the {once} method [instead of using the  {on()} method -due to need it for only one time    ]      :    
